...
export function ListingPriceCard({
  ...
  // Rentals are annual tenancies: no Reserve, the path is message the
  // agent, inspect the property, then pay.
  const isRental = listing.kind === "rental";
  const period = isRental || listing.pricePeriod === "year" ? t.common.year : t.common.night;
  ...
  return (
    <div className="nf-card p-5">
      <p className="flex items-baseline gap-1.5">
        ...
        <span className="text-[0.8125rem] text-[var(--nf-content-muted)]">/ {period}</span>
      </p>

      {!isRental && listing.instantBook && (
      ...
      {isRental && (
        <p className="mt-2 flex items-start gap-1.5 text-[0.78rem] leading-relaxed text-[var(--nf-content-muted)]">
          <UiIcon name="chat-bubble" size={14} className="mt-0.5 shrink-0 text-[var(--nf-electric-300)]" />
          Keep every chat and payment inside RentMe. Anything agreed outside
          the platform is not protected by us.
        </p>
      )}
    </div>
  );
