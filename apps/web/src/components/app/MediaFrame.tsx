 ...
 */
/*
 * DROPPING A BETTER PHOTOGRAPH IN IS THE ONLY STEP, AND IT IS NOT A CODE EDIT.
 *
 * Everything below this comment is the stand-in: two images covering eight
 * scenes, which is the honest state and the weakest part of the visual product.
 * `SCENE_PHOTOGRAPHS` is generated by `scripts/build-scene-manifest.mjs`, which
 * `apps/web` runs before every build. It lists whatever is actually present in
 * `public/brand/scenes/`, named for the scene it belongs to.
 *
 * So a photograph saved as `public/brand/scenes/terrace.avif` becomes the
 * terrace image at the next build, with nothing else changed anywhere. A scene
 * with no file keeps the stand-in, so the eight can be filled one at a time and
 * a missing file never breaks a page.
 *
 * `docs/IMAGERY.md` names a specific, correctly licensed image for each scene,
 * with a direct URL, because this environment answers 403 to every image host
 * and no session can download them.
 *
 * TWO THINGS DO NOT CHANGE WHEN A REAL PHOTOGRAPH ARRIVES.
 *
 *   LAND STILL GETS THE DRAWING. It is not in the manifest's scene list at all.
 *   A plot has nothing built on it and a photograph of a building on a land
 *   listing is the picture contradicting the listing.
 *
 *   THE EXAMPLE MARK STAYS. A real photograph of somebody else's building
 *   standing in for a property it is not a picture of is a stronger claim than
 *   a drawing, not a weaker one, so it needs the label more rather than less.
