...
**Update, 2026-07-30, re-verified against the working tree.** B1, B2, B3, B4,
B5 and B6, all six blockers, are now closed. S1, S2, S4 and S5 closed
alongside them. Each closed row below carries a CLOSED note with what shipped
and where. Nothing else in this document was re-checked as part of that
update; treat every other row as still open unless you verify it yourself.
...
Counts at the time of this audit: **6 BLOCKER, 12 SERIOUS, 10 MINOR.** As of
2026-07-30 all 6 blockers are closed, along with S1, S2, S4 and S5 (see the
CLOSED notes in the tables below). Counts left standing: **0 BLOCKER, 8
SERIOUS, 10 MINOR**, not re-audited beyond the rows marked closed.
...
|---|---------|----------|-------------|
| B1 | **CLOSED 2026-07-30.** ~~Nothing ever grants the `admin` or `super_admin` role.~~ Both signup triggers hardcoded `'user'`; the only role write in application code inserted `'agent'`. No migration, no script, no doc, no seed granted either admin role. | Was: `supabase/migrations/20260728151133_identity_core.sql:81` and `supabase/migrations/20260730013143_signup_trigger_identity_metadata.sql:55`. Closed by `supabase/migrations/20260730090000_admin_bootstrap.sql`: an admin-readable `admin_bootstrap` allow-list table, seeded with the owner's address as `super_admin`, consulted by `handle_new_user()` on signup. | Nobody could enter the admin console, so no listing could go live and no agent application could ever be approved. Now the owner's account arrives elevated on signup with no manual SQL required. |
| B2 | **CLOSED 2026-07-30.** ~~Payment initiation is orphaned.~~ `startCardCheckout` and `payWithWallet` were complete and imported by nothing, so a guest was never offered a way to pay. | Was: `apps/web/src/lib/bookings/checkout.ts:294` and `:454`, imported by nothing outside `lib/bookings/` and `lib/payments/`. Closed by `apps/web/src/app/(app)/checkout/[bookingId]/PayPanel.tsx`, which imports and calls both, and by `ReservePanel.tsx` now linking straight to `/checkout/[bookingId]` as the primary CTA after a reserve. | A guest reserves, and no charge was ever created. Now the checkout page renders `PayPanel` with a working card and wallet path, and reserve success sends the guest there directly. |
| B3 | **CLOSED 2026-07-30.** ~~`confirm()` still has no caller, and neither does the new host console.~~ | Was: `apps/web/src/app/agent/bookings/page.tsx` 11 lines of `AgentComingSoon`; `acceptBooking`/`declineBooking` imported by nothing. Closed by `apps/web/src/app/agent/bookings/BookingsWorkspace.tsx` (441 lines), which imports and calls both, and `apps/web/src/app/agent/earnings/EarningsWorkspace.tsx`, both wired into real pages. | No booking could move `PENDING` to `CONFIRMED`. Now a host has a real queue with Accept and Decline, and a real earnings read. |
| B4 | **CLOSED 2026-07-30.** ~~The Google and Apple sign-in buttons are wired to nothing.~~ They carried no `onClick`, sat in no form and had no `action`. | Was: `apps/web/src/components/auth/AuthPanel.tsx:250`, plain `<button type="button">`. Closed: each provider button is now `<form action={startGoogleOAuth}>` / `<form action={startAppleOAuth}>` (`AuthPanel.tsx:258`), posting to a server action with the provider fixed server side. | The buttons would have become enabled and done nothing the day provider keys landed. Now the handshake begins on the server on click. |
| B5 | **CLOSED 2026-07-30.** ~~Agent identity documents never leave the browser, and the reviewer is shown a count that is always zero.~~ | Was: `apps/web/src/components/agent/ApplyWizard.tsx:61-68`, a `URL.createObjectURL` blob with no storage upload. Closed: the wizard now uploads straight from the browser into a private `agent-documents` bucket under `<uid>/<batch>/...` (`ApplyWizard.tsx:172`), the insert payload carries the paths, and `apps/web/src/lib/admin/queries.ts:365-381` mints a ten-minute signed URL per document for the reviewer. | An applicant's uploads were discarded and the reviewer always saw zero documents. Now the files are stored and the admin reviewer can open each one. |
| B6 | **CLOSED 2026-07-30.** ~~The wallet-payment RPC is called by name but its migration is not in the applied set.~~ | Was: defined only in `supabase/migrations_pending/booking_pay_from_wallet.sql`, outside `supabase/migrations/`. Closed: `supabase/migrations/20260730121229_booking_pay_from_wallet.sql` and `supabase/migrations/20260730124500_wallet_pay_confirmed_unpaid_booking.sql` are both applied; `supabase/migrations_pending/` no longer exists. The function also now accepts a booking that is `PENDING` or `CONFIRMED`-and-unpaid, not `PENDING` only, closing a second bug where a host-accepted stay could never be paid. | Paying for a stay from the wallet used to fail at runtime because the function did not exist. Verified end to end per the commit message: wallet debit, attempt, balanced ledger row, `CONFIRMED` booking, state event, nights closed all land together; replay answers `already_paid` and moves nothing. |
...
| # | Finding | Evidence | Consequence |
|---|---------|----------|-------------|
| S1 | **CLOSED 2026-07-30.** ~~Business applicants are silently filed as individuals and their company details discarded.~~ | Was: `apps/web/src/lib/agent/application.ts:121` hardcoded `type: "individual"` regardless of the wizard's toggle. Closed: `application.ts:209` now writes `type: isBusiness ? "business" : "individual"`, and `business_name`/`business_rc` are written when `isBusiness` (`:219-220`). | A company's registered name and RC number used to vanish between the browser and the database. Now they are filed. |
| S2 | **CLOSED 2026-07-30 (consequence of B3).** ~~The booking notification sends the host to a coming-soon page.~~ | `supabase/migrations/20260729112606_notifications.sql:81` and `:96` still write `href` `'/agent/bookings'`, unchanged, but that route now renders `BookingsWorkspace`, a real console, not `AgentComingSoon`. | A host used to be notified "New booking request", tap it, and land on a placeholder. Now the same link opens a real queue with Accept and Decline. |
| S3 | **`public.reviews` has no writer, and a control promises otherwise.** | Table at `supabase/migrations/20260728152458_engagement.sql`, insert policy at `:109`. Reads only: `apps/web/src/lib/listings/supabase-repository.ts:218` (rating aggregates) and `apps/web/src/lib/profile/queries.ts:76` (a review count on the profile). No insert anywhere in code or SQL. Compounded by `apps/web/src/components/app/bookings/BookingsTabs.tsx:97-103`, a control labelled "Leave a review" that is a `Link` to `/listing/${b.listingId}`. | A guest finishes a stay, taps "Leave a review", arrives at the listing page and finds nothing to review with. Every real listing shows no rating for ever, and the profile's review count is permanently zero. Re-verified 2026-08-04, still open. |
| S4 | **CLOSED 2026-07-30 (consequence of B3).** ~~An approved agent has no way to be paid and no way to see earnings.~~ | Was: `apps/web/src/app/agent/earnings/page.tsx` an 11-line stub; `readAgentEarnings` imported by nothing. Closed: `apps/web/src/app/agent/earnings/EarningsWorkspace.tsx` (157 lines) renders real figures from `readAgentEarnings`. Note: `public.payout_accounts` still has no writer, so an agent can see earnings now but withdrawal-to-bank is a separate, still-open gap. | An agent can now see what has settled from their stays. Paying it out to their bank account is not built. |
| S5 | **CLOSED 2026-07-30 (consequence of B6).** ~~The listing calendar never blocks a booked night.~~ | Was: every writer of `availability` sat behind an unreachable `confirm()` or webhook. Closed: `apps/web/src/lib/bookings/actions.ts` now writes booked nights straight from `reserve` (best effort, the GiST constraint is still what actually prevents the oversell), and `supabase/migrations/20260730121247_release_stale_booking_holds_calendar.sql` makes the 48-hour auto-release delete the nights it wrote, so an abandoned request no longer locks a calendar forever. | Two guests used to be shown the same nights as free. Now a reserved night shows blocked immediately, and an abandoned hold releases its nights on schedule (once `pg_cron` runs the function; see M7, still open). |
...
| M6 | **CLOSED 2026-08-06.** ~~Environment drift in both directions against `apps/web/.env.example`.~~ Closed from two directions at once, and the overlap is worth recording. A parallel session rewrote `apps/web/.env.example` and wrote `docs/ENVIRONMENT.md`, which is the fix for the row as stated. Independently, the wider audit found that this row had understated the problem twice over. First, the **root** `.env.example` was a whole second template that nobody had touched, and it was the more dangerous one: Next.js loads `.env.local` from the application directory, so every value an operator set at the workspace root was read by nothing, with no error to say so. It is now a signpost at the real template. Second, the unread-key list was longer than the six named here. `AUTH_DATABASE_URL`, `GOOGLE_CLIENT_ID/SECRET`, four `APPLE_*`, `X_CLIENT_*`, three `CLOUDINARY_*`, `GOOGLE_MAPS_SERVER_KEY`, `NEXT_PUBLIC_GOOGLE_MAPS_BROWSER_KEY`, `TRAVELGATE_API_KEY`, `NEXT_PUBLIC_SENTRY_DSN` and a `PAYSTACK_WEBHOOK_SECRET` that Paystack does not issue are all read by nothing. Worst of all, `docs/DEPLOY.md` was not merely stale but stated the opposite of the truth as fact: section 2.4 listed six of them as live reads in `lib/auth/providers.ts`, and sections 4.2 and 4.3 instructed the operator to put Google and Apple credentials into Vercel. That module reads exactly one variable, `NEXT_PUBLIC_AUTH_PROVIDERS`, and those credentials belong in the Supabase dashboard. DEPLOY.md 2.4 now says where they really live, 2.5 lists every removal with its reason, and 4.2 and 4.3 say plainly that they used to be wrong. Original finding: | Read by code, absent from the file: `NF_DATA_SOURCE` (the switch that selects seed versus live repositories, the single most consequential setting in the app) and `SUPPORT_MODEL` (`ASSISTANT_MODEL` is documented, this one is not). Declared but read by nothing: `NEXT_PUBLIC_MAPTILER_KEY` (the map uses Carto tiles), `NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY` (funding redirects to hosted checkout, so only the secret key is read), `NEXT_PUBLIC_POSTHOG_KEY`, `NEXT_PUBLIC_POSTHOG_HOST`, `SENTRY_DSN`, `TERMII_API_KEY`. | An operator can set six keys that change nothing, and can miss the one switch that decides whether the app serves seed or live data. `NODE_ENV` and `VERCEL_URL` are platform-provided and correctly absent. |
...
| M9 | **CLOSED.** ~~The public contact form never persists, although a working ticket action exists.~~ `ContactForm.tsx` now posts to `submitContactForm` (`lib/support/actions.ts:219`), which files through `fileSupportTicket`: a real `support_tickets` row, an `NF-SUP` reference on screen and by email, and the row in the queue an admin already works at `/admin/support`. This matters more than a minor row suggests, because `lib/support-email.ts` points all six "contact support" surfaces at `/contact` while no mailbox is configured. That routing is only honest because this write path is live. Original finding: | `apps/web/src/app/(site)/contact/ContactForm.tsx:19-22` sets local state only. `apps/web/src/lib/support/actions.ts:91` (`fileSupportTicket`) already handles anonymous visitors through the service role and would serve it directly. | Minor only because the copy is honest before and after submitting (`ContactForm.tsx:68-72` and `:75-82`). Nobody is misled; there is simply a finished write path sitting one import away. |
...
2. **The admin console and agent listings CRUD both exist.** Section 8 features
   ...
   quality gate. B1, the only reason none of it could function, is closed as
   of 2026-07-30 (see the B1 row above).
...
**Update, 2026-07-30: B1 through B6 are all closed, in the order this section
recommended (B1, B6, B2 and B3 together, B4, B5).** S2 closed as a
consequence of B3. S8 and S3 are still open and are each a few lines; they
remain the two places where the product still actively promises something it
cannot do (a hardcoded unread badge on `/agent/messages`, and a "Leave a
review" control with nothing to review with).
