# SUPERSEDED: Premium Reference Brief, NaijaFinds UI Audit

> # DO NOT BUILD FROM THIS FILE.
>
> **Superseded 2026-08-09. This brief is the direct cause of the visual clutter
> the platform is now removing, and it is retained only as a record of what was
> asked for and why it was wrong.**
>
> It was written for NaijaFinds, a product that no longer exists. It asks, below,
> for tinted icon tiles behind list-row glyphs, symbol effects on every state
> change, a floating pill island tab bar, photographic imagery inside category
> chips, filled-versus-outline variants of every glyph, and neumorphic dual
> shadows in light mode. Several of those were built. An agent that reads this
> file and takes it as a standard will rebuild the exact overload being stripped
> out.
>
> The eight audit files beside this one grade the codebase **against this brief**,
> so all nine inherit its direction and none of them is a backlog.
>
> They are also stale in plain fact. They measured a `globals.css` of 3,167
> lines: it is now 71 lines and 19 ordered partials under
> `apps/web/src/app/css/`. They record that no `components/ui/` directory exists:
> there are 12 primitives in it. They record zero `loading.tsx` files: there are
> 60. They record no ESLint configuration: there is one. **Re-measure every count
> in these nine files before quoting it.**
>
> What replaces this brief: `docs/PRODUCT.md` for what the product is, and
> `RECOMMENDATIONS.md` sections 2 and 3 for the design and navigation work that
> is actually wanted. `RECOMMENDATIONS.md` D-2 names, item by item, which of the
> things asked for below survive on their own merits and which do not.
>
> Everything below this line is the original 2026-08-04 text, unaltered.
